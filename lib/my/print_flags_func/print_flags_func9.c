...
void my_printf_flag_zu(va_list ap, char const *format, int i)
{
    ...
    my_putnbr_base_size_t(nb, "0123456789");
    if (tests.test_dash)
        find_dash(format, i, &nb_to_print);
}

void my_printf_flag_zx(va_list ap, char const *format, int i)
{
    size_t nb = va_arg(ap, size_t);
    ...
    if (tests.test_zero > 0)
        find_zero(format, i, &nb_to_print);
    my_putnbr_base_size_t(nb, "0123456789abcdef");
    if (tests.test_dash)
        find_dash(format, i, &nb_to_print);
}

void my_printf_flag_z_upper_x(va_list ap, char const *format, int i)
{
    size_t nb = va_arg(ap, size_t);
    int nb_to_print = my_nbr_len_base_unsigned(nb, 16);
    tformat_t tests = detect_imcompatible_flag(format, i);
...
void my_printf_flag_tdi(va_list ap, char const *format, int i)
{
...
void my_printf_flag_to(va_list ap, char const *format, int i)
{
    ptrdiff_t nb = va_arg(ap, ptrdiff_t);
    ...
    if (tests.test_zero > 0)
        find_zero(format, i, &nb_to_print);
    my_putnbr_base_ptrdiff_t(nb, "01234567");
    if (tests.test_dash)
        find_dash(format, i, &nb_to_print);
}
